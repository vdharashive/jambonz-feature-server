 ...
 */
  ...
  async exec() {
    ...
    if (this.isNormalCallSession) {
      ...
      this.ttsStreamingBuffer.on(TtsStreamingEvents.Connect, this._onTtsStreamingConnect.bind(this));
    }
    else {
...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _onTtsStreamingConnect(data) {
    this.logger.info({data}, 'CallSession:_onTtsStreamingConnect - tts streaming connection established');
    this.requestor?.request('tts:streaming-event', '/streaming-event', {event_type: 'stream_open'})
      .catch((err) => this.logger.info({err}, 'CallSession:_onTtsStreamingConnect - Error sending stream_open event'));
  }
