...
class TaskGather extends SttTask {
  ...
  async handling(cs, {ep}) {
    ...
    const startListening = async(cs, ep, startTimer = true) => {
    ...
    try {
      if (this.sayTask) {
        ...
        const process = () => {
          ...
          if (!this.killed && !this.resolved) {
            ...
            } else if (!this.sayTask?.isStreamingTts) {
              // For regular TTS with listenDuringPrompt, start the timer now that prompt has finished
              this.logger.debug('Gather:exec - starting timer after say completes for non-streaming TTS');
              this._startTimer();
              if (this.isContinuousAsr && 0 === this.timeout && !this.isBackgroundGather) {
                this._startAsrTimer();
              }
        ...
        const process = () => {
          ...
          if (!this.killed && !this.resolved) {
            if (!this.listenDuringPrompt) {
              ...
              // For play task with listenDuringPrompt, start the timer now that prompt has finished
              this.logger.debug('Gather:exec - starting timer after play completes');
              this._startTimer();
              if (this.isContinuousAsr && 0 === this.timeout && !this.isBackgroundGather) {
                this._startAsrTimer();
              }
      ...
      if (this.listenDuringPrompt) {
        // For streaming TTS, start timer immediately since audio is streamed in real-time
        // For regular TTS, only start transcribing - timer will start when prompt finishes
        const isStreamingTts = this.sayTask?.isStreamingTts || false;
        await startListening(cs, ep, isStreamingTts);
      }
