...
class TaskGather extends SttTask {
  ...
  async handling(cs, {ep}) {
    ...
    const startListening = async(cs, ep) => {
      // Don't restart timer if listenDuringPrompt is true AND speech input - transcription timer was already active
      // For digits-only input, timer was never started during prompt, so start it now
