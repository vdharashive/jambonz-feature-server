...
class TaskGather extends SttTask {
  ...
  async handling(cs, {ep}) {
    ...
    const startListening = async(cs, ep) => {
      // Don't restart timer if listenDuringPrompt is true - transcription was already active
    ...
    try {
      if (this.sayTask) {
        ...
        this.sayTask
          ...
          .then(() => {
            if (this.sayTask.isStreamingTts && !this.sayTask.closeOnStreamEmpty) return;
  ...
  updateTaskInProgress(opts) {
    ...
    const {timeout} = opts;
    this.timeout = timeout === 0 ? 0 : (timeout || 15) * 1000;
    this._startTimer();
    return true;
  }
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (evt.is_final) {
      if (evt.alternatives[0].transcript.trim() === '' && !this.callSession.callGone && !this.killed) {
      ...
      if (this.vendor === 'deepgram') {
        ...
        if (evt.alternatives[0].transcript.trim() === '') {
          emptyTranscript = true;
        }
      ...
      if (!emptyTranscript && evt.alternatives[0].transcript.trim() !== '') {
      ...
      if (this._timeoutTimer && !emptyTranscript && evt.alternatives[0].transcript.trim() !== '') {
        this._startTimer();
      }
