...
class TtsStreamingBuffer extends Emitter {
  ...
  async _feedQueue(handlingTimeout = false) {
    ...
    try {
      ...
      this.logger.debug(`TtsStreamingBuffer:_feedQueue sending chunk to tts: ${modifiedChunk} with flush`);

      try {
        await this._api(this.ep, [this.ep.uuid, 'flush']);
      } catch (err) {
        this.logger.info({ err, chunk }, 'TtsStreamingBuffer:_feedQueue Error sending TTS chunk with flush');
      }
