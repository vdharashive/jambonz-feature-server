...
const TIMEOUT_RETRY_MSECS = 250; // 1 second
...
class TtsStreamingBuffer extends Emitter {
  ...
  async _feedQueue(handlingTimeout = false) {
    ...
    try {
      ...
      try {
        ...
        // If we just sent a complete sentence, send a flush command to ensure immediate processing
        if (isCompleteSentence) {
          this.logger.debug('TtsStreamingBuffer:_feedQueue sending flush after complete sentence');
          await this._doFlush();
        }
      } catch (err) {
  ...
  async _onConnect(vendor) {
    ...
    // Emit event to notify that the TTS streaming connection has been established
    this.emit(TtsStreamingEvents.Connect, { vendor });
