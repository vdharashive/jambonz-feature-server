...
class WsRequestor extends BaseRequestor {
  ...
  _clearPendingMessages() {
    for (const [msgid, obj] of this.messagesInFlight) {
      ...
      clearTimeout(timer);
      if (!this._initMsgId) {
        const errorMsg = `abandoning msgid ${msgid} since socket is closed`;
        const bindings = this.logger.bindings ? this.logger.bindings() : {};
        this.logger.info({
          call_sid: this.call_sid,
          ...
          requestorId: this.id,
          msgid
        }, `WsRequestor:_clearPendingMessages - ${errorMsg}`);
        obj.failure(errorMsg);
      }
    }
